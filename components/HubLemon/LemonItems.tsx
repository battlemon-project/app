...
import { useLemonStore, ItemType } from "../../helpers/lemonStore";
...
export default function LemonItems() {
  const { lemons, activePlatform, wearingItem } = useLemonStore(({ lemons, activePlatform, wearingItem }) => ({ lemons, activePlatform, wearingItem }), shallow)
  ...
  const refreshItems = async () => {
    ...
    const items = itemsOwnedNfts.ownedNfts.map(nft => nft.rawMetadata.properties).filter(x => x);
    setLemonItems(items);
    setInventoryLoader(false);
  }
  ...
  const { config } = usePrepareContractWrite(mintItemData(address, lemonItems.length))
  ...
  return (<>
      ...
      { inventoryLoader ? 
        ...
        <div className="row align-self-start w-100">
          { currentItemsFilter == 'dressed' ? <>
            {lemonItems.filter(item => item.attachedTo == lemons.ownedNfts[activePlatform - 1].tokenId).map((item, idx) => 
              <div className={`col col-3 border px-1 px-1 ${ item.id == selectedItem?.id && 'selected' }`} key={`${item.type}${idx}`}>
                <div className="link text-center py-2" onClick={clickToItem(item)} style={{cursor: 'pointer'}}>
                  <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/128/Icon_${item.name}_128.png`} alt={item.type} className="img-fluid" />
                </div>
              </div>
            )}
            { lemonItems.filter(item => item.attachedTo == lemons.ownedNfts[activePlatform - 1].tokenId).length == 0 ?
            ...
            {lemonItems.filter(item => (!currentItemsFilter || currentItemsFilter == item.type) && !item.attachedTo).map((item, idx) => 
              <div className={`col col-3 border px-1 px-1 ${ item.id == selectedItem?.id && 'selected' }`} key={`${item.type}${idx}`}>
                <div className="link text-center py-2" onClick={clickToItem(item)} style={{cursor: 'pointer'}}>
                  <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/128/Icon_${item.name}_128.png`} alt={item.type} className="img-fluid" />
                </div>
    ...
    <div className="position-absolute" style={{bottom: '-70px'}}>
      { currentItemsFilter != 'dressed' && <div className="d-flex mt-2 bottom-buttons">
        ...
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'cap' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'cap' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('cap')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_cap_64.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'belt' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'belt' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('belt')}>
        ...
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'cloth' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'cloth' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('cloth')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_cloth_64.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'mask' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'mask' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('mask')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_mask_64.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'glasses' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'glasses' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('glasses')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_face_64.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'shoes' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'shoes' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('shoes')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_foot_64.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'back' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'back' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('back')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_back_128.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'fire_arms' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'fire_arms' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('fire_arms')}>
          <img src={`${process.env.NEXT_PUBLIC_STATIC}/assets/tiny/icon_hand_r_64.png`} />
        </a>
        <a className={`col col-auto ${lemonItems.find(item => item.type == 'cold_arms' && !item.attachedTo) ? 'enabled' : 'disabled' } ${ currentItemsFilter == 'cold_arms' ? 'active' : '' }`} href={'#'} onClick={filterOutifts('cold_arms')}>
