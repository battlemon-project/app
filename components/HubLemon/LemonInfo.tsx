...
export default function LemonInfo() {
  const { lemons, activePlatform } = useLemonStore(({ lemons, activePlatform }) => ({ lemons, activePlatform }), shallow)
  ...
  return (<>
    <div className="inventory-scroll d-flex">
      <div className="px-4 pb-2 pt-3">
        ...
        <h5>Characters:</h5>
        {lemons?.ownedNfts[activePlatform - 1].rawMetadata?.properties.map((property) => {
          if (!property?.name) return null;
          return <div key={property.type}>
