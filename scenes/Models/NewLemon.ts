...
export const NewLemon = async (scene: Scene, lastLemon: SuiData | null): Promise<void> => {
  ...
  let weapon = (await SceneLoader.ImportMeshAsync("", "/glb/", "BTLMN_Outfit_Weapon_A.glb", scene)).meshes[0];
  weapon.parent = scene.getMeshByName('placeholder_weapon_r') as TransformNode

  let cap = (await SceneLoader.ImportMeshAsync("", "/glb/", "BTLMN_Outfit_Cap_A.glb", scene)).meshes[0];
  cap.parent = scene.getMeshByName('placeholder_cap') as TransformNode

  let cloth = (await SceneLoader.ImportMeshAsync("", "/glb/", "BTLMN_Outfit_Cloth_A.glb", scene)).meshes[0];
  cloth.parent = scene.getMeshByName('placeholder_cloth') as TransformNode
  
  let back = (await SceneLoader.ImportMeshAsync("", "/glb/", "BTLMN_Outfit_Back_A.glb", scene)).meshes[0];
  back.parent = scene.getMeshByName('placeholder_back') as TransformNode

  let face = (await SceneLoader.ImportMeshAsync("", "/glb/", "BTLMN_Outfit_Face_A.glb", scene)).meshes[0];
