...
const LemonsColdArms: `0x${string}` = '0x77d2Ae81dF753A49481Bcbffcf95646Be792c9D7'
const LemonsFireArms: `0x${string}` = '0x65d3c393E7958a86aAC74675C01e11D408a42A6f'
...
export async function getLemons(address: string): Promise<LemonOwnedNftsResponse> {
  const result = await alchemy.nft.getNftsForOwner(address, { contractAddresses: [Lemons] }) as LemonOwnedNftsResponse;
...
export async function getItems(address: string): Promise<ItemOwnedNftsResponse> {
  return await alchemy.nft.getNftsForOwner(Lemons, { contractAddresses: [
    LemonsCap, LemonsBack, LemonsGlasses, LemonsColdArms, LemonsFireArms, LemonsBelt, LemonsShoes
...
export const mintItemData = (address: `0x${string}` | undefined, length: number | undefined) =>{
  const mint = {
    functionName: 'mint',
    args: [1]
  }
